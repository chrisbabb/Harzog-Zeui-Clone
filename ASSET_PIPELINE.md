...
| TeamColorSlots     | **live**: `AssetResolver` applies the team body material to any mesh named `TeamColor`, `TeamColor_01`, ... and the team emissive to `TeamGlow*` / `EmissiveStrip*` when a final asset loads. The `Hull` node's material override additionally remains the damage-flash target | all |
...
- **Material naming:** `M_<asset>_<part>`, e.g. `M_tank_hull`,
  ...
  integration. Team coloring is handled at load time: `AssetResolver`
  assigns team body/emissive material overrides to `TeamColor*` /
  `TeamGlow*` / `EmissiveStrip*` nodes (§2), so those meshes need no
  authored team materials at all. One remaining caveat: the *damage
  flash* targets the `Hull` node's material override, which imported
  glTF materials don't populate — set the hull material as an override
  in the `.tscn` wrapper, or that per-hit flash silently skips the asset
...
`AssetResolver.gd` is the single seam, and the fallback is **already
wired** — gameplay code (`Unit.gd`, `Base.gd`, `Outpost.gd`, and the
title-screen diorama) only ever calls it:

1. `AssetResolver.instantiate_unit_visual(unit_type, team)` /
   `instantiate_building_visual(building_type, team)` resolve the asset
   path (`get_unit_asset_path` / `get_building_asset_path`; existence
   checks via `has_final_unit_asset` / `has_final_building_asset`).
2. Candidates are checked in order:
   `assets/art/final/<subdir>/<file_base>.tscn` (artist-tuned wrapper),
   then `<file_base>.glb`. The first hit is instantiated as the visual
   root, and team materials are applied to the `TeamColor*` /
   `TeamGlow*` / `EmissiveStrip*` slots (§2). An asset with none of
   those slots still loads — the skip is logged once per asset.
3. If no candidate exists (or the file fails to load — also warned once,
   never crashing a spawn), the procedural placeholder is generated by
   `UnitVisualFactory` / `BuildingVisualFactory` exactly as before.
